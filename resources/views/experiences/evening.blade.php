@php
  $eveningImage = '/wp-content/uploads/2026/08/36_202309051441007901-bcf6382f-142f-4570-85ee-9cdb14a05cc3-scaled.webp';
  $eveningAlt = __('The lake at Satori at day\'s end', 'sage');
@endphp

<section class="bg-[#f7f3eb] pb-24">
  <div class="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-16">
    <div class="mx-auto max-w-3xl text-center" data-reveal>
      <p class="text-[0.75rem] uppercase tracking-[0.3em]">
        <span class="text-brand-gold">05</span>
        <span class="mx-2 text-brand-ink/30" aria-hidden="true">—</span>
        <span class="text-brand-muted">{{ __('Evening', 'sage') }}</span>
      </p>

      <h2 class="mt-5 font-heading text-3xl font-light leading-tight text-brand-ink sm:text-4xl">
        {{ __('Bonfire, stars, and the return to quiet', 'sage') }}
      </h2>

      <p class="mx-auto mt-6 max-w-2xl text-base leading-8 text-brand-muted sm:text-[16px]">
        {{ __('As the light goes, Satori becomes stiller and more intimate. Bonfires are lit. The hills fall silent. The sky opens properly, in a way it no longer does near a city. In season, fireflies. Through the year, stargazing, open-air film nights and sunset setups by the pool.', 'sage') }}
      </p>

      <p class="mx-auto mt-6 max-w-2xl text-base leading-8 text-brand-muted sm:text-[16px]">
        {{ __('This is the hour for slower dinners, warm conversation, and the sense that the day has asked nothing of you except that you were present for it.', 'sage') }}
      </p>
    </div>

    <div class="group relative mt-14 overflow-hidden" data-reveal>
      <img class="aspect-[4/3] w-full object-cover transition-transform duration-[1200ms] ease-out group-hover:scale-105" src="{{ $eveningImage }}" alt="{{ $eveningAlt }}">
    </div>
  </div>
</section>
